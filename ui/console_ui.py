...
class EmployeeConsoleUI:
    """Handles CLI presentation, user prompts, input validation loops, and ASCII table views."""
    ...
    def prompt_input(
        ...
        """Prompts for input and validates it. Continues until valid input or cancellation."""
        while True:
            try:
    ...
    def display_employee_table(self, employees: List[Employee]) -> None:
        """Prints employee records in an aligned, structured ASCII table."""
        if not employees:
        ...
        border = (
            f"+-{'-' * col_widths['id']}-+-{'-' * col_widths['name']}-+-"
            f"{'-' * col_widths['email']}-+-{'-' * col_widths['dept']}-+-"
            f"{'-' * col_widths['desg']}-+-{'-' * col_widths['date']}-+"
        )

        header_row = (
            f"| {col_headers['id'].ljust(col_widths['id'])} | "
        ...
        for emp in employees:
            row = (
                f"| {emp.employee_id.ljust(col_widths['id'])} | "
    ...
    def delete_employee_flow(self) -> None:
        ...
        confirm = input(
            f"{COLOR_WARNING}Are you sure you want to delete {emp.name} (ID: {emp.employee_id})? (y/N): {COLOR_RESET}"
        ).strip().lower()
